...
def test_report_initialization_and_metrics():
  ...
  dp = DataProvider(
    {
      ...
      "close": np.array([1.0, 1.05, 1.10, 1.08, 1.15,  # Symbol "a"
                         1.0, 1.02, 1.04, 1.03, 1.06]), # Symbol "b"
      "ror": np.array([0.0, 0.01, 0.02, 0.015, 0.03,    # Symbol "a" ror
                       0.0, 0.02, 0.04, 0.03, 0.06]),   # Symbol "b" ror
  ...
  assert np.array_equal(report.close, np.array([1.0, 1.02, 1.04, 1.03, 1.06]))
  assert np.array_equal(report.ror, np.array([0.0, 0.02, 0.04, 0.03, 0.06]))
  assert np.array_equal(report.actions, np.array([0, 1, 0, 2, 0]))

  # Calculate metrics
  report.calcaute_metrics(dp)
  ...
  assert np.allclose(report.avg_win, 0.03)
